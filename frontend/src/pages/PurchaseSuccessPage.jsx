...
const PurchaseSuccessPage = () => {
	...
	useEffect(() => {
		const handleCheckoutSuccess = async (sessionId) => {
			try {
				...
				setError(error.response?.data?.message || "Failed to process your payment");
	...
	if (isProcessing) return (
		<div className="h-screen flex flex-col items-center justify-center">
			...
			<p className="text-lg text-gray-300">Processing your payment...</p>
		</div>
	);
		...
		<div className="h-screen flex flex-col items-center justify-center px-4">
			<div className="max-w-md w-full bg-gray-800 rounded-lg shadow-xl overflow-hidden">
				<div className="p-6 sm:p-8">
					...
					<div className="flex justify-center">
						<Link to="/" className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-2 rounded-md transition-colors">
							Return to Home
						</Link>
					</div>
				</div>
